            ...
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="good in goodsList" :key="good.id">
                <div class="list-wrap">
                  ...
                  <div class="operate">
                    <router-link  target="_blank" class="sui-btn btn-bordered btn-danger" to="/shopcart">加入购物车</router-link>
...
import {mapGetters,mapState} from 'vuex'
  ...
  export default {
    ...
    computed:{
      // mapGetters里面的写法：传递的数组，因为咱们的getters计算是没有划分模块的
    ...
    methods:{
      ...
      // 把这次请求封装为一个函数，当需要的时候调用就行
      getData(){
        this.$store.dispatch('getSearchList',this.searchParams)
      },
      // 删除分类的名字，面包屑
      removeCategoryName(){
        // 把他带给服务器的参数置空了，但是还需要向服务器发请求
        //带给服务器的参数说明时可有可无的，如果属性值为空的字符串还是会把相应的字段带给服务器
        // 但是把相应的字段变为undefined，当前的这个字段不会带给服务器
         this.searchParams.categoryName=undefined
         this.searchParams.category1Id=undefined
         this.searchParams.category2Id=undefined
         this.searchParams.category3Id=undefined
         this.getData()
        // 地址栏也需要修改：进行路由跳转(现在路由跳转只是从自己跳到自己)
        // 严谨：本意是删除query，如果路径中存在params参数不因该删除，路由跳转的时候应该带上
      ...
      // 删除关键字的面包屑
      removeKerwordName(){
        // 将服务器参数的searchParams中的keyword置空
      ...
      //排序的操作
      changeOrder(flag){
        // flag形参：是一个标记，代表用户点击的是综合(1)还是价格(2),用户点击传递进来的
        // 起始状态
        let originOreder=this.searchParams.order
    ...
    watch:{
      ...
      // 分类名字和关键字不用清除，因为每一次路由的变化都会赋予新的值
