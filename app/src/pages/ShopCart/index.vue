  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCkeck&&cartInfoList.length>0" @click="updateAllcartChecked">
        <span>全选</span>
      </div>
      <div class="option">
...
import throttle from 'lodash/throttle'
  export default {
        ...
        switch(type){
          case 'add':
           ...
           disNum=1
            break;
          case 'minus':
            // 判断产品的个数大于1，才能传递给服务器-1  产品的个数小于等于1,则传0(原封不动)
          ...
          case 'change':
            if(isNaN(disNum)||disNum<1){
              ...
              // 正常的情况(小数取整) 带个服务器的变化量 用户输入的值-产品的起始个数
    ...
    async updateChecked(cart,event){
          // 带个服务器的参数ischecked不是布尔值，应该是0|1
          let checked=event.target.checked?'1':'0'
        try {
          // 修改商品的选中状态后派发请求，再次更新购物车
   ...
   async deleteAllCheckedCart(){
      //  派发一个action
      try {
       await this.$store.dispatch('deleteAllCheckedAcrt')
      //  再发请求，获取购物车数据
        this.getData()
      } catch (error) {
        console.log(error.message)
  ...
  async  updateAllcartChecked(event){
    ...
    computed:{
      ...
      // 计算底部全选是否选中(全部产品都选中，才勾选)
      isAllCkeck(){
        // 遍历数组里面原理：只要全部元素isChecked属性为1，则返回true
        // 只要有一个为假，则返回false
