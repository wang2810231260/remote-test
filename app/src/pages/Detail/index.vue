        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳到B路由，这里在接入购物车，进行路由跳转之前，发送请求
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      skuImageList(){
        // 如果服务器的数据没有回来，skuInfo是一个空对象
        return this.skuInfo.skuImageList||[]
      }
    },
    methods:{
      changeActive(saleAttr,arr){
        //  遍历全部售卖属性值isChecked为0没有高亮
        arr.forEach(item=>{
          item.isChecked=0
        })
      ...
      // 添加购物车
     async addShopCar(){
        // 1.派发action---将产品加入到数据库（通知服务器)
        /*
        当前这里派发一个action，也想服务器发请求，判断加入购物车成功还是失败，进行相应的操作
        this.$store.dispatch('addOrUpdateShopCar',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        上面这行代码说白了：调用了仓库中的addOrUpdateShopCar函数，而addOrUpdateShopCar的返回值是一个promise对象
        */ 
      try{
      ...
      // 产品信息数据(比较复杂),通过会话存储(不持久化，会话结束了，数据再消失)
      // 不管是本地存储还是会话存储，一般存储的都是字符串
           sessionStorage.setItem('skuInfo',JSON.stringify(this.skuInfo))
         this.$router.push({name:'addcardsuccess',query:{skuNum:this.skuNum}}) 
      } catch(error){
        alert(error.message)
      }     
      }
