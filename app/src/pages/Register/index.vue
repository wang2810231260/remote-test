<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="password" placeholder="请输入密码" v-model="password1" name="password1" v-validate="{ required:true,is:password}" :class="{invalid:errors.has('password1')}">   
        <span class="error-msg">{{errors.first('password1')}}</span>
      </div>
      <div class="controls">
        <input type="checkbox" v-model="agree" name="agree" v-validate="{ required:true,tongyi:agree}" :class="{invalid:errors.has('agree')}">   
    ...
    methods: {
      // 获取验证码
       async  getCode(){
        ...
        // 用户注册
       async userRegister(){
        // 表单全部验证完成,向服务器发起请求，进行注册，只要有一个表单错误，就不会发请求
